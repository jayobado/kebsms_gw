...
class SmsAdtelCommand extends CConsoleCommand
{
    ...
    public function run($args)
    {
		...
		$validity='';
        $status='';
 	$inboxes=Yii::app()->adtel->createCommand("SELECT * FROM sms WHERE clicked=0  AND shortcode='20023' ORDER BY sms_index DESC")->queryAll();
	...
	foreach ($inboxes as $sms)
      {
		...
		$query=pg_escape_string(trim($sms['Message']));
		...
		$response.= ', Status: '.$status.'.';
              ...
              //$msg=pg_escape_string(trim($response));
			  $msg=$this->truncate(pg_escape_string(trim($response)));
        
           $send=Yii::app()->adtel->createCommand("INSERT INTO request(Number,Message,productid) VALUES('$sender','$msg','kebs')")->execute();
              
                 if($send){
                 Yii::app()->adtel->createCommand("UPDATE sms SET clicked=1 WHERE sms_index=$id")->execute();}
          }
...
$response='Product: '.$product.', Firm: '.$firm.', DM Issue Date: '.$issue_date.', Expiry Date: '.$expiry_date.', Status: '.$validity.'.';
                 ...
                 else
                 {
                     ...
                     if($expiry_date < $currentdate): 
                      ...
                      $validity='Permit is valid';
						 ...
						 $response='Product: '.$product.', Brand: '.$brand.', Firm: '.$firm.', SM Issue Date: '.$issue_date.', Expiry Date: '.$expiry_date.', '.$validity.'.';
	...
	$response='Product: '.$product.', Brand: '.$brand.', Firm: '.$firm.', SM Issue Date: '.$issue_date.', Expiry Date: '.$expiry_date.','.$validity.'.';
           ...
           $send=Yii::app()->adtel->createCommand("INSERT INTO request(Number,Message,productid) VALUES('$sender','$msg','kebs')")->execute();
                 ...
                 if($send){Yii::app()->adtel->createCommand("UPDATE sms SET clicked=1 WHERE sms_index=$id")->execute();}
          }
