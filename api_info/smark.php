...
$sql = "SELECT * FROM firmlisting WHERE product_name='$s_brand' OR product_brand='$s_brand' OR product_id='$s_brand' ";
  $result = $db->query($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        $issue_date = ", SM Issue Date : ".$row["issue_date"];
        ...
        if ($expiryD >= $currentdate) {
            $gen_all = $product_name.$product_brand.$company_name.$issue_date.$expiry_date.$status1;
            $messageerror = "[{\"message\":\"$gen_all\"}]";
            echo "{\"items\":$messageerror}";
        } else {
            $gen_all = $product_name.$product_brand.$company_name.$issue_date.$expiry_date.$status2;
            $messageerror = "[{\"message\":\"$gen_all\"}]";
            echo "{\"items\":$messageerror}";
        }
    } else {
